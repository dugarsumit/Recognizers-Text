...
from ...resources import GermanDateTime
from .datetimeperiod_parser_config import GermanDateTimePeriodParserConfiguration


class GermanDateTimePeriodParser(BaseDateTimePeriodParser):
    def __init__(self, config: GermanDateTimePeriodParserConfiguration):
    ...
    def parse_specific_time_of_day(self, source: str, reference: datetime) -> DateTimeResolutionResult:
        ...
        start_index = len(GermanDateTime.Tomorrow) if trimmed_text.startswith(
            GermanDateTime.Tomorrow) else 0
        ...
        matches = list(RegExpUtility.get_safe_reg_exp(
            GermanDateTime.TimeOfDayRegex).finditer(trimmed_text[start_index:]))
        if matches:
            match = matches[0]
