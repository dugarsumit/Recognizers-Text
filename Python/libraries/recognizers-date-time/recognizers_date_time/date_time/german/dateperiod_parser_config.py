...
class GermanDatePeriodParserConfiguration(DatePeriodParserConfiguration):
    ...
    def is_month_to_date(self, source: str) -> bool:
        trimmed_source = source.strip().lower()
        return any(trimmed_source.endswith(o) for o in GermanDateTime.MonthToDateTerms)

    def is_week_only(self, source: str) -> bool:
        trimmed_source = source.strip().lower()
        return any(trimmed_source.endswith(o) for o in GermanDateTime.WeekTerms) and not\
